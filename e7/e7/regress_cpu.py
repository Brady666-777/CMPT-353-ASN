...
def get_data(filename):
    ...
    # target is temperature at next time step
    sysinfo[y_column] = sysinfo['temperature'].shift(-1)
    # drop last row where no next temperature
...
def get_trained_coefficients(X_train, y_train):
    """
    Create and train a model based on the training data. Return the model and coefficients.
    """
...
def output_regression(coefficients):
    regress = ' + '.join(f'{coef:.3}*{col}' for coef, col in zip(coefficients, X_columns))
...
def smooth_test(coef, sysinfo, outfile):
    ...
    # Identity transition, then replace first row with regression coefs
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        print(f"Usage: python {sys.argv[0]} <train.csv> <valid.csv>")
        sys.exit(1)
