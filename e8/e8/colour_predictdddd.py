...
# map labels to representative RGB colours for plotting
COLOUR_RGB = {
    'red': (255, 0, 0),
    'orange': (255, 112, 0),
    'yellow': (255, 255, 0),
...
def main():
    ...
    def to_lab(arr):
        return rgb2lab(arr.reshape(-1, 1, 1, 3)).reshape(-1, 3)

    def to_hsv(arr):
        return np.array([colorsys.rgb_to_hsv(*pix) for pix in arr])

    # build models
    bayes_rgb_model = GaussianNB()
    bayes_convert_model = Pipeline([
        ('lab', FunctionTransformer(to_lab)),
        ('nb', GaussianNB())
    ])

    knn_rgb_model = KNeighborsClassifier(n_neighbors=5)
    knn_convert_model = Pipeline([
        ('hsv', FunctionTransformer(to_hsv)),
        ('knn', KNeighborsClassifier(n_neighbors=5))
    ])

    rf_rgb_model = RandomForestClassifier(n_estimators=100, random_state=42)
    rf_convert_model = Pipeline([
        ('lab', FunctionTransformer(to_lab)),
